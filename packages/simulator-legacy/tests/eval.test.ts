...
describe("evalExpr — unary ops", () => {
	test("UnaryMinus negates", () => {
		expect(
			evalExpr(
				{ type: "UnaryMinus", operand: { type: "Number", value: 5 } },
				makeCtx(),
			),
		).toBe(-5);
	});
	...
	test("Not of 0 returns 1", () => {
		expect(
			evalExpr(
				{ type: "Not", operand: { type: "Number", value: 0 } },
				makeCtx(),
			),
		).toBe(1);
	});
	...
	test("Not of non-zero returns 0", () => {
		expect(
			evalExpr(
				{ type: "Not", operand: { type: "Number", value: 7 } },
				makeCtx(),
			),
		).toBe(0);
	});
