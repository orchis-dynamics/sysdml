...
import {
	...
	isAllowedWorkerUrl,
} from "../../src/security/trusted-types.js";

describe("isAllowedWorkerUrl", () => {
	test("accepts a same-origin https assets path", () => {
		expect(
			isAllowedWorkerUrl("https://demo.example.com/assets/worker-abc.js"),
		).toBe(true);
	});
	...
	test("accepts a vscode-webview-resource asset path", () => {
		expect(
			isAllowedWorkerUrl(
				"https://abc-def.vscode-webview.net/path/assets/worker-abc.js",
			),
		).toBe(true);
	});
	...
	test("accepts a hashed worker bundle URL with query string", () => {
		expect(
			isAllowedWorkerUrl("https://demo.example.com/assets/worker-abc.js?t=123"),
		).toBe(true);
	});

	test("rejects http:// from a different host", () => {
		expect(isAllowedWorkerUrl("http://attacker.example.com/evil.js")).toBe(
			false,
		);
	});
	...
	test("accepts blob: URLs (required for VS Code webview cross-origin worker workaround)", () => {
		...
		expect(isAllowedWorkerUrl("blob:vscode-webview://abc-def/12345")).toBe(
			true,
		);
	});

	test("rejects URLs without .js extension", () => {
		expect(isAllowedWorkerUrl("https://demo.example.com/assets/worker")).toBe(
			false,
		);
	});
});
...
describe("installTrustedTypesPolicy", () => {
	...
	test("the registered createScriptURL accepts allowed URLs", () => {
		...
		installTrustedTypesPolicy();
		expect(
			capturedRules?.createScriptURL(
				"https://demo.example.com/assets/worker-abc.js",
			),
	...
	test("the registered createScriptURL rejects untrusted URLs", () => {
		...
		installTrustedTypesPolicy();
		expect(() => capturedRules?.createScriptURL("javascript:alert(1)")).toThrow(
			/Refused to create script URL/,
		);
	});
