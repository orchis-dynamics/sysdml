...
function buildInitEnv(
  ...
  const evalCtx = buildEvalContext(initEnv, simCtx, initEnv, initEnv, gfRegistry);
  for (const auxVar of orderedAux) initEnv[auxVar.id] = evalExpr(auxVar.expr, evalCtx);
  for (const flow of orderedFlows) initEnv[flow.id] = evalExpr(flow.rate, evalCtx);
  return initEnv;
}
