...
function interpolate(
  ...
  if (input <= xPoints[0]) return extrapolateLeft(kind, xPoints, yPoints, input);
  if (input >= xPoints[last]) return extrapolateRight(kind, xPoints, yPoints, input);

  const bracketIndex = findBracketIndex(xPoints, input);
  if (kind === 'step') return yPoints[bracketIndex];
  return linearInterp(
    xPoints[bracketIndex], yPoints[bracketIndex],
    xPoints[bracketIndex + 1], yPoints[bracketIndex + 1],
    input,
  );
}
...
function extrapolateLeft(
  ...
  if (kind === 'extra') return linearInterp(xPoints[0], yPoints[0], xPoints[1], yPoints[1], input);
  return yPoints[0];
}
...
function extrapolateRight(
  ...
  const last = xPoints.length - 1;
  if (kind === 'extra') return linearInterp(xPoints[last - 1], yPoints[last - 1], xPoints[last], yPoints[last], input);
  return yPoints[last];
}
...
function findBracketIndex(xPoints: number[], input: number): number {
  ...
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    if (xPoints[mid] <= input) lo = mid; else hi = mid - 1;
  }
  return lo;
}

function linearInterp(x0: number, y0: number, x1: number, y1: number, x: number): number {
  if (x1 === x0) return y0;
