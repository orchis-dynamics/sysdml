...
function transformFunctionCall(
	...
	switch (name) {
		case "DELAY1":
			return desugarDelay1(args, state);
		case "DELAY3":
			return desugarDelay3(args, state);
		case "DELAYN":
			return desugarDelayN(args, state);
		case "SMTH1":
			return desugarSmth1(args, state);
		case "SMTH3":
			return desugarSmth3(args, state);
		case "SMTHN":
			return desugarSmthN(args, state);
		case "TREND":
			return desugarTrend(args, state);
		case "FORCST":
			return desugarForcst(args, state);
		default:
			return {
				type: "FunctionCall",
				name,
...
function makeStockOutputExpr(
	stockId: string,
	delayTimeExpr: IRExpressionNode,
): IRExpressionNode {
	return {
...
function desugarDelay3(
	...
	const stage1 = desugarDelay1(
		buildStageArgs(inputExpr, stageDelay, initExpr),
		state,
	);
	const stage2 = desugarDelay1(
		buildStageArgs(stage1, stageDelay, initExpr),
		state,
	);
	return desugarDelay1(buildStageArgs(stage2, stageDelay, initExpr), state);
}

function desugarDelayN(
	args: readonly IRExpressionNode[],
	state: DesugarState,
): IRExpressionNode {
	...
	for (let i = 0; i < n; i++) {
		current = desugarDelay1(
			buildStageArgs(current, stageDelay, initExpr),
			state,
		);
	}
	return current;
...
function desugarSmth1(
	...
	const flowRate: IRExpressionNode = {
		...
		left: {
			...
			op: "-",
			left: inputExpr,
			right: { type: "Reference", id: stockId },
		},
		right: avgTimeExpr,
	};
...
function desugarSmth3(
	...
	const stage1 = desugarSmth1(
		buildStageArgs(inputExpr, stageTime, initExpr),
		state,
	);
	const stage2 = desugarSmth1(
		buildStageArgs(stage1, stageTime, initExpr),
		state,
	);
	return desugarSmth1(buildStageArgs(stage2, stageTime, initExpr), state);
}

function desugarSmthN(
	args: readonly IRExpressionNode[],
	state: DesugarState,
): IRExpressionNode {
