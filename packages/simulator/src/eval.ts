...
export function evalExpr(node: IRExpressionNode, evalCtx: EvalContext): number {
	switch (node.type) {
		case "Number":
			return node.value;
		case "Reference":
			return evalCtx.env[node.id] ?? 0;
		case "UnaryMinus":
			return -evalExpr(node.operand, evalCtx);
		case "Not":
			return evalExpr(node.operand, evalCtx) === 0 ? 1 : 0;
		case "BinaryOperation":
			return evalBinOp(
				node.op,
				evalExpr(node.left, evalCtx),
				evalExpr(node.right, evalCtx),
			);
		case "IfThenElse":
			return evalIfThenElse(
				node.cond,
				node.thenBranch,
				node.elseBranch,
				evalCtx,
		...
		case "GraphicalFunctionCall":
			return evalGFCall(node.name, node.argument, evalCtx);
		case "FunctionCall":
			return evalFunctionCall(node.name, node.args, evalCtx);
	}
}
...
function flooredMod(dividend: number, divisor: number): number {
	...
	return (
		remainder + (Math.sign(remainder) !== Math.sign(divisor) ? divisor : 0)
	);
}
...
function evalGFCall(
	name: string,
	argNode: IRExpressionNode,
	evalCtx: EvalContext,
): number {
	const graphicalFunction = evalCtx.gfRegistry.get(name);
	if (graphicalFunction === undefined)
		throw new Error(`Unknown graphical function '${name}'`);
