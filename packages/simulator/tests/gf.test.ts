...
function linearGf(xscale: [number, number], ypts: number[]): IRGraphicalFunction {
  return { id: 'f', kind: 'linear', xscale, xpts: null, ypts, yscale: null };
}

function extraGf(xscale: [number, number], ypts: number[]): IRGraphicalFunction {
...
describe('gfLookup — linear (continuous)', () => {
  ...
  test('exact left endpoint returns first y', () => expect(gfLookup(gf, 0)).toBeCloseTo(0));
  test('exact right endpoint returns last y', () => expect(gfLookup(gf, 1)).toBeCloseTo(1));
  test('midpoint interpolates correctly', () => expect(gfLookup(gf, 0.25)).toBeCloseTo(0.25));
  test('three-quarter point', () => expect(gfLookup(gf, 0.75)).toBeCloseTo(0.75));
  test('below range clamps to first y', () => expect(gfLookup(gf, -1)).toBeCloseTo(0));
  test('above range clamps to last y', () => expect(gfLookup(gf, 2)).toBeCloseTo(1));
});
...
describe('gfLookup — extrapolate', () => {
  ...
  test('below range extrapolates (x=-1 → y=-1)', () => expect(gfLookup(gf, -1)).toBeCloseTo(-1));
  test('above range extrapolates (x=2 → y=2)', () => expect(gfLookup(gf, 2)).toBeCloseTo(2));
});
...
describe('gfLookup — extrapolate with non-unit slope', () => {
  ...
  test('above range (x=3): extrapolate → 6', () => expect(gfLookup(gf, 3)).toBeCloseTo(6));
  test('below range (x=-1): extrapolate → -2', () => expect(gfLookup(gf, -1)).toBeCloseTo(-2));
});
...
describe('gfLookup — step (discrete)', () => {
  ...
  test('at x=0.9 still returns first y (step, not linear)', () => expect(gfLookup(gf, 0.9)).toBe(10));
  ...
  test('below range clamps to first y', () => expect(gfLookup(gf, -1)).toBe(10));
...
describe('gfLookup — explicit xpts', () => {
  ...
  test('at x=0.5 returns 10 (exact point)', () => expect(gfLookup(gf, 0.5)).toBeCloseTo(10));
  test('at x=0.25 interpolates to 5', () => expect(gfLookup(gf, 0.25)).toBeCloseTo(5));
  test('at x=0.75 interpolates to 15', () => expect(gfLookup(gf, 0.75)).toBeCloseTo(15));
});

describe('gfLookup — edge cases', () => {
  test('single point returns that y for any input', () => {
    const gf: IRGraphicalFunction = { id: 'f', kind: 'linear', xscale: [0, 0], xpts: null, ypts: [42], yscale: null };
