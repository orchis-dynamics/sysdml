...
aux d = DELAY3(s, 3)
    `);
		const { ir: desugared } = desugarIR(ir);
		const hiddenStocks = desugared.stocks.filter((s) =>
			s.id.startsWith("_delay1"),
		);
...
aux d = DELAYN(s, 3, 0)
    `);
		...
		expect(diagnostics.some((d) => d.code === "INVALID_DELAY_ORDER")).toBe(
			true,
		);
	});
});
...
aux smoothed = SMTH1(s, 4)
    `);
		const { ir: desugared, diagnostics } = desugarIR(ir);
		expect(diagnostics).toHaveLength(0);
		const hiddenStocks = desugared.stocks.filter((st) =>
			st.id.startsWith("_smth1"),
		);
