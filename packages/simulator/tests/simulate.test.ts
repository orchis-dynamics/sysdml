...
function evalAux(
	exprSrc: string,
	extraAux: Record<string, string> = {},
): number {
	const auxLines = Object.entries(extraAux)
		.map(([id, e]) => `aux ${id} = ${e}`)
		.join("\n");
	const src = `
sfd m
...
describe("logical operators evaluate correctly", () => {
	...
	test("NOT (0 - 0) → 1 (zero is falsy)", () =>
		expect(evalAux("NOT (0 - 0)")).toBe(1));
	...
	test("5 AND 7 → 1 (any non-zero is true)", () =>
		expect(evalAux("5 AND 7")).toBe(1));
...
describe("exponentiation evaluates correctly", () => {
	...
	test("2 ^ 3 ^ 2 → 512 (right-assoc: 2^(3^2) = 2^9)", () =>
		expect(evalAux("2 ^ 3 ^ 2")).toBe(512));
	test("-2 ^ 2 → -4 (unary minus binds less tightly than ^)", () =>
		expect(evalAux("-2 ^ 2")).toBe(-4));
	test("a ^ b with a=3 b=4 → 81", () =>
		expect(evalAux("a ^ b", { a: "3", b: "4" })).toBe(81));
});
...
describe("MOD evaluates with Knuth floored semantics", () => {
	...
	test("-7 MOD 3 → 2 (sign follows divisor)", () =>
		expect(evalAux("-7 MOD 3")).toBe(2));
	test("7 MOD -3 → -2 (sign follows divisor)", () =>
		expect(evalAux("7 MOD -3")).toBe(-2));
	test("-7 MOD -3 → -1 (both negative, sign follows divisor)", () =>
		expect(evalAux("-7 MOD -3")).toBe(-1));
});
