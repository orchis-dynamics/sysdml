...
export function buildIR(src: string): IR {
  ...
  if (parseDiagnostics.length > 0) throw new Error(`Parse: ${parseDiagnostics[0].message}`);
  const { ir, diagnostics: irDiagnostics } = compileAST(ast!);
  if (irDiagnostics.length > 0) throw new Error(`IR: ${irDiagnostics[0].message}`);
  return ir!;
}
...
export function runModel(src: string): SimRow[] {
  ...
  if (diagnostics.some(d => d.code === 'CYCLE_IN_AUX')) throw new Error('Cycle in aux');
  return rows;
}
...
export function runExpr(exprSrc: string, steps: number, step = 1): SimRow[] {
  ...
  const src = modelSrc(`aux result = ${exprSrc}`, `start: 0 end: ${end} step: ${step}`);
  return runModel(src);
}
